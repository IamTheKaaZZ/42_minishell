...
/*   Updated: 2021/10/27 14:50:13 by bcosters         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool	retval_or_limiter(char *limiter, char **line, int retval)
{
	...
	{
		ft_strdel(line);
		return (err_handler("get next line", 1, true));
	}
...
bool	unlink_tmp(char *error)
{
	if (unlink(TEMPFILE) < 0)
		return (err_handler("unlink", 1, true));
	else if (error != NULL)
		return (err_handler(error, 1, false));
	else
		return (true);
}
...
static bool	init_here_doc(t_file *tmp, char **line)
{
	*line = NULL;
	...
	if (tmp->fd == -1)
		return (err_handler("creation of temporary file", 1, true));
